...
**`ns_haptics_build_raw_tables()`** fills **`out`** with the same curves **`ns_haptics_init()`** loads internally.

Helpers include **`ns_haptics_freq_index_to_hz_hi` / `_lo`**, **`ns_haptics_exp_lut_index_to_amplitude_linear`**, **`ns_haptics_host_amp_index_to_exp_lut_index`**, **`ns_haptics_get_basic()`**, and optional PCM stepping helpers (**`ns_haptics_hz_to_phase_increment_fp`**, etc.).
